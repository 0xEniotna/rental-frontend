...
export default function Rental({ item, nfts }: Props) {
  ...
  useEffect(() => {
    fetch(`https://api-testnet.aspect.co/api/v0/assets?owner_address=${getAddressFromString(item)}`, options)
  ...
  // const { data: priceToPay, loading: priceLoading } = useStarknetCall({
  //   contract: rental,
  //   method: "getPriceToPay",
  //   args: [],
  // });

  // DEPOSIT PARAMS: nft_id: Uint256, _nft_address: felt  
  // const calls = useMemo(() => {
  //   const tx = 
  //   return Array(1).fill(tx)
  // }, [state.account, rentalAddress])
